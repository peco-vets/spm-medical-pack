---
name: django-tdd
description: pytest-django を用いた Django のテスト戦略、TDD メソドロジー、factory_boy、モック、カバレッジ、DRF API のテスト（Django, pytest-django, TDD, factory_boy）。
origin: ECC
---

# Django テスト × TDD

pytest、factory_boy、Django REST Framework を用いた Django アプリのテスト駆動開発である。

## 起動タイミング

- 新規 Django アプリの記述
- Django REST Framework API の実装
- Django のモデル・ビュー・シリアライザのテスト
- Django プロジェクトのテスト基盤セットアップ
...
- **factory を使う**: 手動オブジェクト生成の代わりに
- **テスト1件につき1アサーション**: フォーカスを保つ
- **記述的なテスト名**: `test_user_cannot_delete_others_post`
- **エッジケースをテスト**: 空入力・None 値・境界条件
- **外部サービスはモック**: 外部 API に依存しない
- **fixture を使う**: 重複を排除
- **権限をテスト**: 認可が動作することを保証
- **テストを高速化**: `--reuse-db` と `--nomigrations` を使う

### 非推奨事項

- **Django 内部はテストしない**: Django を信頼する
- **サードパーティコードをテストしない**: ライブラリを信頼する
- **失敗テストを無視しない**: すべてのテストはパスする必要がある
- **テスト間に依存を作らない**: 任意の順で実行可能にする
- **過度にモックしない**: 外部依存のみモックする
...
| パターン | 用途 |
|---------|-------|
| `@pytest.mark.django_db` | DB アクセスを有効化 |
| `client` | Django テストクライアント |
| `api_client` | DRF API クライアント |
| `factory.create_batch(n)` | 複数オブジェクト生成 |
| `patch('module.function')` | 外部依存のモック |
| `override_settings` | 設定を一時変更 |
| `force_authenticate()` | テストで認証をバイパス |
| `assertRedirects` | リダイレクトの検証 |
| `assertTemplateUsed` | テンプレート使用の検証 |
| `mail.outbox` | 送信済みメールの検証 |

備考: テストはドキュメントである。良いテストはコードがどう動くべきかを説明する。シンプル・可読・メンテナブルに保つこと。
