...
description: Django のセキュリティベストプラクティス、認証、認可、CSRF 防御、SQL インジェクション防止、XSS 防止、セキュアなデプロイ設定（Django security, auth, CSRF, SQL injection, XSS）。
origin: ECC
---

# Django セキュリティベストプラクティス

一般的な脆弱性から Django アプリを保護するための包括的なセキュリティガイドラインである。
...
| 項目 | 説明 |
|-------|-------------|
| `DEBUG = False` | 本番で DEBUG を有効化しない |
| HTTPS 限定 | SSL 強制、セキュア cookie |
| 強力なシークレット | SECRET_KEY に環境変数を使う |
| パスワードバリデーション | すべてのパスワードバリデータを有効化 |
| CSRF 防御 | デフォルトで有効、無効化しない |
| XSS 防止 | Django は自動エスケープ、ユーザー入力に `&#124;safe` を使わない |
| SQL インジェクション | ORM を使う、クエリ内で文字列連結しない |
| ファイルアップロード | ファイル種別とサイズを検証 |
| レート制限 | API エンドポイントをスロットル |
| セキュリティヘッダー | CSP、X-Frame-Options、HSTS |
| ロギング | セキュリティイベントを記録 |
| 更新 | Django と依存パッケージを最新に保つ |

備考: セキュリティは製品ではなくプロセスである。セキュリティ慣行を定期的に見直し、更新すること。
