---
name: csharp-testing
description: xUnit、FluentAssertions、モッキング、統合テスト、テスト組織化のベストプラクティスを使った C# と .NET テストパターン (C# testing, xUnit, FluentAssertions, NSubstitute, Moq, integration tests, Testcontainers, WebApplicationFactory)。
origin: ECC
---

# C# テストパターン

xUnit、FluentAssertions、モダンテストプラクティスを使った .NET アプリケーションのための包括的テストパターン。

## 起動するタイミング

- C# コードの新しいテストを書く
- テスト品質とカバレッジをレビュー
- .NET プロジェクトのテストインフラをセットアップ
- 不安定または遅いテストをデバッグ
...
|---|---|
| 実装詳細をテスト | 挙動と結果をテスト |
| 共有可変テスト状態 | テストごとに新インスタンス (xUnit はコンストラクタで自動) |
| 非同期テストで `Thread.Sleep` | タイムアウト付き `Task.Delay` またはポーリングヘルパー |
| `ToString()` 出力でアサート | 型付きプロパティでアサート |
| テストごとに 1 つの巨大アサーション | テストごとに 1 つの論理アサーション |
| 実装を記述するテスト名 | 挙動で命名: `Method_ExpectedResult_WhenCondition` |
| `CancellationToken` を無視 | 常に渡しキャンセルを検証 |
