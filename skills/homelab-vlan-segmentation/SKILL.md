...
description: UniFi、pfSense/OPNsense、MikroTik を用いて IoT、guest、trusted、server トラフィック向けにホームネットワークを VLAN にセグメント化（VLAN segmentation, homelab, UniFi, pfSense, MikroTik）。スイッチ trunk 構成、firewall ルール、ワイヤレス SSID マッピングを含む。
origin: community
---

# ホームラボ VLAN セグメンテーション

ホームネットワークを隔離 VLAN に分割し、IoT デバイス・ゲスト・メイン PC が互いに通信できないようにする方法である。ホームネットワークに対する最も影響の大きいセキュリティアップグレードである。

ここで示すすべての firewall ルールはセグメント間に隔離を追加するもので、既存保護を削除しない。メンテナンスウィンドウで変更を適用し、次へ進む前に各ステップ後にセグメント間の接続性を検証する。

## 利用タイミング

- 初めてホームネットワークに VLAN をセットアップする
- IoT デバイス（スマート電球、カメラ、TV）を trusted デバイスから隔離する
- 家庭デバイスに到達できないゲスト Wi-Fi ネットワークを作成する
- 概念に不慣れな相手に VLAN がどう動くかを説明する
- trunk ポート、access ポート、SSID-VLAN マッピングを構成する
- pfSense/OPNsense/UniFi で inter-VLAN ルーティングまたは firewall ルール問題のトラブルシューティング
...
### UniFi Firewall ルール（Traffic Rules）
...
- Trusted・IoT・Servers・Guest の4 VLAN から開始し、必要に応じて追加する
- Pi-hole を Servers VLAN（192.168.30.x）に置く
- すべての VLAN から Pi-hole IP の DNS（port 53）を許可する firewall ルールを — 任意の RFC1918 block ルールの前に追加する
- 各ルール変更後に隔離をテストする: IoT VLAN から trusted デバイスへ ping を試み、失敗するべきである
- スイッチと AP の Web UI 向けに管理 VLAN を使い、アクセスを Trusted VLAN のみに制限する
- VLAN 設計を表（VLAN ID、name、subnet、purpose）にドキュメント化する
