...
description: "OpenClaw AI エージェント向けの完全な「龍蝦の魂」スキームを鍛造する。ユーザー設定またはガチャに基づき、アイデンティティ位置づけ、魂の説明 (SOUL.md)、キャラクター化されたボトムライン規則、名前、アバター画像生成プロンプトを出力する。現在の環境に承認済み画像生成スキルが提供されている場合、統一スタイルのアバター画像を自動生成可能。OpenClaw 龍蝦の魂を作成・設計・カスタマイズする必要がある場合に使用 (Forges complete lobster soul schemes for OpenClaw AI Agent; outputs identity, soul description SOUL.md, character-based rules, name, avatar prompts)。"
origin: community
---
