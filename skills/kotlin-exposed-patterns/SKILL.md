...
description: JetBrains Exposed ORM パターン (DSL クエリ、DAO パターン、トランザクション、HikariCP コネクションプール、Flyway マイグレーション、リポジトリパターンを含む) (JetBrains Exposed ORM patterns including DSL queries, DAO pattern, transactions, HikariCP connection pooling, Flyway migrations, repository pattern)。
origin: ECC
---

# Kotlin Exposed パターン

JetBrains Exposed ORM によるデータベースアクセスの包括的パターン (DSL クエリ、DAO、トランザクション、本番対応の設定を含む)。

## 使用するタイミング

- Exposed でデータベースアクセスをセットアップする場合
- Exposed DSL または DAO で SQL クエリを記述する場合
- HikariCP でコネクションプールを設定する場合
- Flyway でデータベースマイグレーションを作成する場合
- Exposed でリポジトリパターンを実装する場合
- JSON カラムや複雑なクエリを扱う場合

## 動作の仕組み

Exposed は 2 つのクエリスタイルを提供する: 直接的な SQL ライク式のための DSL と、エンティティライフサイクル管理のための DAO である。HikariCP は `HikariConfig` で設定された再利用可能なデータベース接続のプールを管理する。Flyway は起動時にバージョン管理された SQL マイグレーションスクリプトを実行し、スキーマを同期させる。すべてのデータベース操作はコルーチン安全性と原子性のために `newSuspendedTransaction` ブロック内で実行する。リポジトリパターンは Exposed クエリをインターフェースの背後にラップし、ビジネスロジックがデータ層から疎結合のままになり、テストではインメモリ H2 データベースを使用できるようにする。
...
### kotlinx.serialization による JSONB
...
| パターン | 説明 |
|---------|-------------|
| `object Table : UUIDTable("name")` | UUID 主キーでテーブル定義 |
| `newSuspendedTransaction { }` | コルーチン安全なトランザクションブロック |
| `Table.selectAll().where { }` | 条件付きクエリ |
| `Table.insertAndGetId { }` | 挿入して生成 ID を返す |
| `Table.update({ condition }) { }` | 一致行を更新 |
| `Table.deleteWhere { }` | 一致行を削除 |
| `Table.batchInsert(items) { }` | 効率的な一括挿入 |
| `innerJoin` / `leftJoin` | テーブル結合 |
| `orderBy` / `limit` / `offset` | ソートとページネーション |
| `count()` / `sum()` / `avg()` | 集計関数 |

**注意**: 単純なクエリには DSL スタイルを使用し、エンティティライフサイクル管理が必要な場合は DAO スタイルを使う。コルーチンサポートには必ず `newSuspendedTransaction` を使用し、テスト可能性のためにデータベース操作をリポジトリインターフェースの背後にラップすること。
