...
description: コルーチン、null 安全性、DSL ビルダーを用いた堅牢で効率的・保守可能な Kotlin アプリケーション構築のための慣用的な Kotlin パターン、ベストプラクティス、規約 (Idiomatic Kotlin patterns, best practices, conventions for building robust, efficient, maintainable Kotlin applications with coroutines, null safety, DSL builders)。
origin: ECC
---

# Kotlin 開発パターン

堅牢で効率的・保守可能なアプリケーション構築のための慣用的な Kotlin パターンとベストプラクティス。

## 使用するタイミング

- 新規 Kotlin コードの記述
- Kotlin コードのレビュー
- 既存 Kotlin コードのリファクタリング
- Kotlin モジュールやライブラリの設計
- Gradle Kotlin DSL ビルドの設定

## 動作の仕組み

このスキルは 7 つの主要領域にわたって慣用的な Kotlin 規約を強制する: 型システムと安全呼び出し演算子による null 安全性、`val` とデータクラスの `copy()` による不変性、網羅的な型階層のためのシールドクラスとインターフェース、コルーチンと `Flow` による構造化並行性、継承なしで振る舞いを追加するための拡張関数、`@DslMarker` とラムダレシーバーによる型安全な DSL ビルダー、ビルド設定のための Gradle Kotlin DSL である。
...
Kotlin の型システムは null 許容型と非 null 型を区別する。これを最大限活用する。
...
`var` より `val` を、可変コレクションより不変コレクションを優先する。
...
| イディオム | 説明 |
|-------|-------------|
| `val` over `var` | 不変変数を優先 |
| `data class` | equals/hashCode/copy 付きの値オブジェクト用 |
| `sealed class/interface` | 制限された型階層用 |
| `value class` | ゼロオーバーヘッドの型安全ラッパー |
| 式 `when` | 網羅的パターンマッチング |
| 安全呼び出し `?.` | null 安全なメンバーアクセス |
| エルビス `?:` | null 許容型のデフォルト値 |
| `let`/`apply`/`also`/`run`/`with` | クリーンコードのためのスコープ関数 |
| 拡張関数 | 継承なしで振る舞いを追加 |
| `copy()` | データクラスの不変更新 |
| `require`/`check` | 前提条件アサーション |
| コルーチン `async`/`await` | 構造化並行実行 |
| `Flow` | コールドリアクティブストリーム |
| `sequence` | 遅延評価 |
| デリゲーション `by` | 継承なしで実装を再利用 |
...
**注意**: Kotlin コードは簡潔だが読みやすくあるべきである。安全性のために型システムを活用し、不変性を優先し、並行性のためにコルーチンを使うこと。迷ったらコンパイラに助けてもらうこと。
