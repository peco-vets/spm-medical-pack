...
description: Pi-hole のインストール、ブロックリスト管理、DNS-over-HTTPS セットアップ、DHCP 統合、ローカル DNS レコード、ホームネットワークでの DNS 解決問題のトラブルシューティング（Pi-hole, DNS, blocklist, DoH, homelab）。
origin: community
---

# ホームラボ Pi-hole DNS

Pi-hole は Raspberry Pi や任意の Linux ホストで動くネットワーク全体 DNS 広告ブロッカである。ネットワーク上のすべてのデバイスが自動的に広告・マルウェアドメインブロッキングを得られる — ブラウザ拡張不要である。

## 利用タイミング

- Raspberry Pi または Linux ホストへの Pi-hole インストール
- ホームネットワークの DNS サーバとしての Pi-hole 構成
- ブロックリストの追加・管理
- DNS-over-HTTPS（DoH）アップストリームリゾルバ設定
- ローカル DNS レコード作成（例 `nas.home.lan`、`pi.home.lan`）
- Pi-hole インストール後にインターネットアクセスを失うデバイスのトラブルシューティング
- DHCP と並行・置換で Pi-hole を稼働させる
...
Docker は Pi-hole の最も簡単なインストール方法で、更新とバックアップが容易になる。
...
デプロイ前に `<pinned-release-tag>` を現行 Pi-hole リリースタグに置換する。長期 DNS インフラには `latest` を避け、アップグレードを意図的・レビュー可能にする。

`PIHOLE_WEBPASSWORD` は `docker-compose.yml` の隣の `.env` で設定し、`600` に chmod し、git から除外する — compose ファイルに直接置かない。

Web admin: `http://<pi-ip>/admin`

### ベアメタルインストール（Raspberry Pi OS / Debian / Ubuntu）

Pi-hole はインストール前に静的 IP を要する。
...
## DNS-over-HTTPS アップストリーム

DoH は DNS クエリを暗号化し、ISP に解決サイトを見せない。
...
サービスを名前で到達可能にする（例 `nas.home.lan`、`grafana.home.lan`）。

> **ドメイン名注記:** `.home.lan` はホームラボで広く使われ実用上動作する。IETF 予約のローカル用サフィックスは `.home.arpa`（RFC 8375）— 標準に従うならこれを使う。Pi-hole の DNS レコードに `.local` を使うのは mDNS/Bonjour と衝突するため避ける。
...
- Pi-hole インストール前に Pi に静的 IP または DHCP 予約を与える
- Pi-hole を主 DNS とする。冗長性が必要なら、厳格ブロッキング時に公衆リゾルバではなく第2の Pi-hole を追加する
- 暗号化アップストリームクエリのため cloudflared で DoH を有効化する
- ローカルドメインを `home.lan` に設定し、すべてのサービスに DNS レコードを作る
- Query Log を時折レビューする — ブロックされたクエリでデバイスが何をしているか見える
