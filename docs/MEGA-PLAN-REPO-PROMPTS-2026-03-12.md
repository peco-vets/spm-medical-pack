# Mega Plan リポジトリプロンプトリスト — 2026 年 3 月 12 日

## 目的

これらのプロンプトを使い、残る 3 月 11 日メガプラン作業をリポジトリ別に分割する。並列エージェント向けに書かれており、3 月 12 日オーケストレーション・Windows CI レーンは `#417` 経由で既にマージされていることを前提とする。

## 現状スナップショット

- `everything-claude-code` はオーケストレーション、Codex ベースライン、Windows CI 回復レーンを完了した。
- 次にオープンな ECC Phase 1 アイテム:
  - `#399` のレビュー
  - 繰り返しのディスカッション圧力を追跡対象 Issue に変換
  - selective-install アーキテクチャの定義
  - ECC 2.0 ディスカバリドキュメントの執筆
- `agentshield`、`ECC-website`、`skill-creator-app` はすべて `main` worktree が dirty であり、`main` で直接編集すべきでない。
- `applications/` はスタンドアロン git リポジトリではない。`<ECC_ROOT>` の親ワークスペースリポジトリ内に存在する。

## リポジトリ: `everything-claude-code`

### Prompt A — PR `#399` レビューとマージレディネス
...
### Prompt C — ECC 2.0 ディスカバリとアダプタ仕様
...
### Prompt — Skill インポートパイプラインとプロダクトフィット
...
現行 worktree は、ここに存在する dirty なスキルファイル変更に触れない ECC ネイティブ Phase 1 作業に留まるべき。最善の次のローカルタスク:

1. selective-install アーキテクチャ
2. ECC 2.0 ディスカバリドキュメント
3. PR `#399` レビュー
